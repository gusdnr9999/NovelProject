...
public class Server implements Runnable {
	...
	class Client extends Thread {
		...
		/*
		 * https://search.daum.net/search?w=tot&DA=YZR&t__nil_searchbox=btn&q=%EC%9E%90%
		 * EB%B0%94
		 */
			...
			try {
				while (true) {
					...
					case Function.LOGIN: {
						...
						// 1. 전체적으로 로그인 정보 전송
						messageAll(Function.LOGIN + "|" + id + "|" + nickname + "|" + gender);
						// 2. 입장메세지 전송
						messageAll(Function.WAITCHAT + "|[알림]" + nickname + "님이 입장하셨습니다");
						// 3. waitVc에 저장
						waitVc.add(this);
						// => 메인화면으로 이동
						...
						for (Client client : waitVc) {
							messageTo(Function.LOGIN + "|" + client.id + "|" + client.nickname + "|" + client.gender);
						}
						// 5. 방정보 전송
					}
						break;
					case Function.WAITCHAT: {
						messageAll(Function.WAITCHAT + "|[" + nickname + "] " + st.nextToken());
					}
						break;
					case Function.EXIT: {
						messageAll(Function.EXIT + "|" + id);
						messageAll(Function.WAITCHAT + "|[알림]" + nickname + "님이 퇴장하셨습니다");
